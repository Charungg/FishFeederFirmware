...
int selectSetupScheduleTimeOption(textData* scheduleTimeTexts, textData* dayTexts, feedingScheduleData* feedingScheduleData, feedingSchedule* feedingSchedule, int* currentOption){
    ...
    switch(*currentOption){
        ...
        case 2:
            incrementScheduleTimeFunctionPtr = &updateScheduleHour;
            runningModifyScheduleTimeText(incrementScheduleTimeFunctionPtr, scheduleTimeTexts, dayTexts, feedingSchedule, currentOption);
            break;

        case 3:
            incrementScheduleTimeFunctionPtr = &updateScheduleMinute;
            runningModifyScheduleTimeText(incrementScheduleTimeFunctionPtr, scheduleTimeTexts, dayTexts, feedingSchedule, currentOption);
            break;
        case 4:
            incrementScheduleTimeFunctionPtr = &updateScheduleSecond;
            runningModifyScheduleTimeText(incrementScheduleTimeFunctionPtr, scheduleTimeTexts, dayTexts, feedingSchedule, currentOption);
            break;
...
void addSchedule(feedingScheduleData* feedingScheduleData, feedingSchedule* newSchedule){
    ...
    if (feedingScheduleData -> arrayOfSchedules == NULL){
        printf("Schedule ptr is null\n");
        feedingScheduleData -> arrayOfSchedules = (feedingSchedule*)malloc(sizeof(feedingSchedule));
    }

    else{
        printf("Resizing array\n");
        feedingScheduleData -> arrayOfSchedules = realloc(feedingScheduleData -> arrayOfSchedules, sizeof(feedingSchedule)*(feedingScheduleData -> amountOfSchedule + 1));
    }

    feedingScheduleData -> arrayOfSchedules[feedingScheduleData -> amountOfSchedule] = *newSchedule;
    feedingScheduleData -> amountOfSchedule += 1;

    //To-DO
    // Check which one is the next feed time

    printf("Size of array of Schedules: %d\n", feedingScheduleData -> amountOfSchedule);
    for (int i = 0; i < feedingScheduleData -> amountOfSchedule; i++){
        printf("schedulesArrayPtr[%d]:\n", i);
        printf("DAY OF WEEK: %d\n", feedingScheduleData -> arrayOfSchedules[i].dayOfWeek);
        printf("SCHEDULE TIME: %02d:%02d:%02d\n", feedingScheduleData -> arrayOfSchedules[i].time -> hour, feedingScheduleData -> arrayOfSchedules[i].time -> minute, feedingScheduleData -> arrayOfSchedules[i].time -> second);
        printf("NUMBER OF FEED: %d\n", feedingScheduleData -> arrayOfSchedules[i]. numberOfFeed);
    }
