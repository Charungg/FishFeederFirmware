...
int clockSecond(void);
int clockMinute(void);
int clockHour(void);
int clockDay(void);
int clockMonth(void);
int clockYear(void);
int clockDayOfWeek(void); // Sunday = 0, Monday = 1, etc
...
void motorStep(void); // rotates the feeder container one step (1 degree)
void foodFill(int foodLevel); // set food level. range 1 - 60%

// button function
// returns one of "SHORT_PRESS" "LONG_PRESS" "NO_PRESS"
char* buttonState(void); // note caller must dispose of char* result
...
int jniSetup(void); // setup the JavaFX GUI and then run userProcessing() once GUI is initialised
int javaFx(void); // start the JavaFX GUI - must be called after jniSetup returns when GUI quits
